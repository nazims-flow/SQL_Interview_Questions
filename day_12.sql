...
(2172, 145, '2024-05-15 00:00:00', 43001, 8000),
(3023, 578, '2024-05-18 00:00:00', 25852, 3000),
(4934, 145, '2024-05-28 00:00:00', 43001, 9000),
...
SELECT user_id,
       ...
       FROM purchases
	   WHERE EXTRACT(MONTH FROM date_of_purchase)=EXTRACT(MONTH FROM current_timestamp - interval '1 month')
	   GROUP BY 1
...
WITH CTE AS
     (SELECT *,
	  ...
	  SELECT 
	  department,
	  ROUND(AVG(upd_end_date-start_date)::NUMERIC , 2)  as AVG_DUR
	  FROM CTE 
	  GROUP BY 1
